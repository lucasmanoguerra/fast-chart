...
pub enum InputEvent {
    // --- Mouse ---
    /// Mouse cursor moved to screen position.
    ...
    // --- Stylus ---
    /// Stylus/pen contact moved.
    StylusMove(StylusEvent),
    /// Stylus/pen pressed down.
...
pub struct WheelEvent {
    ...
    pub delta_x: f64,
    /// Vertical scroll delta (negative = up / zoom in, positive = down / zoom out).
    pub delta_y: f64,
    /// X position of cursor when wheel event occurred.
    pub x: f64,
    /// Y position of cursor when wheel event occurred.
    pub y: f64,
    /// Modifier keys held during the event.
    pub modifiers: ModifierState,
}
...
pub struct PinchEvent {
    ...
    pub center_y: f64,
    /// Distance between fingers (in screen pixels).
    pub distance: f64,
    /// Scale factor relative to the previous pinch frame (1.0 = no change).
    pub scale: f64,
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct KeyEvent {
    /// Physical key code (platform-independent, see `KeyCode`).
    pub key: KeyCode,
    /// Modifier keys held during the event.
    pub modifiers: ModifierState,
...
pub enum KeyCode {
    ...
    // --- Letter keys used as shortcuts ---
    Letter(char),

    // --- Modifier keys (for detecting combos) ---
    Shift,
    Control,
    Alt,
    Super,
    ...
    /// Unknown key not in this mapping.
    Unknown,
}
...
impl InputEvent {
    ...
    /// Create a mouse move event with modifiers.
    ...
    /// Create a left mouse button up event.
    ...
    /// Create a wheel event with modifiers.
    ...
    pub fn touch_start(id: u64, x: f64, y: f64) -> Self {
        InputEvent::TouchStart(TouchEvent {
            id,
            x,
            y,
            pressure: None,
    ...
    pub fn touch_move(id: u64, x: f64, y: f64) -> Self {
        InputEvent::TouchMove(TouchEvent {
            id,
            x,
            y,
            pressure: None,
    ...
    pub fn touch_end(id: u64, x: f64, y: f64) -> Self {
        InputEvent::TouchEnd(TouchEvent {
            id,
            x,
            y,
            pressure: None,
