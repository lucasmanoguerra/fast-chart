...
use fc_app::app::layout::{LayoutEngine, VerticalStack};
use fc_app::app::layout_manager::LayoutManager;
use fc_app::app::pane::events::PaneEventBus;
use fc_primitives::Rect;
use fc_app::ports::data_provider::{DataError, DataEvent, DataProvider};
...
use fc_domain::marker::{Marker, MarkerPosition, MarkerSet, MarkerShape};
use fc_domain::price_line::{LineStyle, PriceLine, PriceLineId, PriceLineSet};
use fc_domain::price_scale::{DefaultPriceFormatter, PriceFormatter, PriceScaleId};
...
fn markers_remove_by_id() {
    ...
    assert!(markers.remove(&fc_domain::marker::MarkerId("a".to_string())));
    assert_eq!(markers.len(), 1);
    assert!(markers.get(&fc_domain::marker::MarkerId("a".to_string())).is_none());
...
fn price_lines_builder_chaining() {
    let line = PriceLine::new("entry", 150.0)
        ...
        .with_label("Entry")
        .with_label_position(fc_domain::price_line::LabelPosition::Center);
...
fn divider_drag_resize_integration() {
    ...
    bus.push(fc_app::app::pane::events::PaneEvent::DividerDragged {
        index: 0,
        delta: 30.0,
    });

    // Simulate resulting pane resize
    bus.push(fc_app::app::pane::events::PaneEvent::PaneResized {
        id: 1,
        new_height: 0.55,
    });
    ...
    match &events[0] {
        fc_app::app::pane::events::PaneEvent::DividerDragged { index, delta } => {
            assert_eq!(*index, 0);
    ...
    match &events[1] {
        fc_app::app::pane::events::PaneEvent::PaneResized { id, new_height } => {
            assert_eq!(*id, 1);
...
fn price_scale_locked_no_autofit_integration() {
    let mut ps = PriceScale::new(
        ...
        PriceScaleOptions {
            auto_scale: true,
            mode: fc_domain::price_scale::PriceScaleMode::Locked,
            ..Default::default()
        },
    );
    ps.value_min = 50.0;
...
fn pane_add_remove_roundtrip() {
    ...
    bus.push(fc_app::app::pane::events::PaneEvent::PaneAdded { id: 3 });
    bus.push(fc_app::app::pane::events::PaneEvent::PaneAdded { id: 4 });
    bus.push(fc_app::app::pane::events::PaneEvent::PaneRemoved { id: 3 });
    ...
    let removed: Vec<_> = events
        .iter()
        .filter(|e| matches!(e, fc_app::app::pane::events::PaneEvent::PaneRemoved { .. }))
        .collect();
...
fn volume_z_index_is_500() {
    ...
    let cmds = v.update(&[], Rect::new(0.0, 200.0, 800.0, 200.0));
    for cmd in &cmds {
        if let fc_app::render::commands::DrawCommand::DrawRect { z_index, .. } = cmd {
...
use fc_app::render::indicator_renderer::IndicatorRenderer;
use fc_app::render::commands::DrawCommand as DC;

struct MockIndicator;

impl fc_app::render::series_renderer::SeriesRenderer for MockIndicator {
    ...
    fn hit_test(&self, _x: f32, _y: f32) -> Option<fc_app::render::series_renderer::SeriesHit> { None }
...
impl IndicatorRenderer for MockIndicator {
    fn render_overlay(&self, pane_bounds: Rect) -> Vec<DC> {
        vec![DC::DrawLine {
            ...
            width: 2.0,
            style: fc_app::render::commands::LineStyle::Solid,
            z_index: 700,
        }]
    }
...
fn overlay_mode_default_overlay_on_pane_0() {
    ...
    impl fc_domain::Indicator<100> for SMA {
        fn calculate(&self, _: &fc_primitives::series::TimeSeries<fc_primitives::Bar, 100>) -> fc_primitives::series::TimeSeries<f64, 100> {
            fc_primitives::series::TimeSeries::new()
        }
        fn name(&self) -> &str { "SMA" }
    }
    let sma = SMA;
    assert_eq!(sma.overlay_mode(), fc_domain::indicator::OverlayMode::OverlayOnPane(0));
...
fn overlay_mode_separate_pane_integration() {
    ...
    impl fc_domain::Indicator<100> for RSI {
        ...
        fn overlay_mode(&self) -> fc_domain::indicator::OverlayMode {
            fc_domain::indicator::OverlayMode::SeparatePane
        }
    }
    let rsi = RSI;
    assert_eq!(rsi.overlay_mode(), fc_domain::indicator::OverlayMode::SeparatePane);
}
