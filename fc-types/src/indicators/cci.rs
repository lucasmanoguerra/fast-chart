...
impl Indicator<MAX_SERIES_LEN> for Cci {
    fn calculate(&self, series: &TimeSeries<Bar, MAX_SERIES_LEN>) -> TimeSeries<f64, MAX_SERIES_LEN> {
        ...
        // Rolling CCI for subsequent bars
...
/// Compute mean deviation of `tp_values[start..start+period]` from `sma`.
...
mod tests {
    ...
    fn cci_basic() {
        ...
        let cci = Cci { period: 20 };
        let result = cci.calculate(&bars);
        // 50 bars, period 20 → 50 - 20 + 1 = 31 outputs
        assert_eq!(result.len(), 31);
    }

    #[test]
    fn cci_constant_prices() {
        // When all prices are constant, TP = SMA, mean_dev = 0, CCI = 0
